...
// Updated FIELD_TYPES to include select and checkbox
...
export default function FieldMastersPage() {
  ...
  const validateFieldUniqueness = () => {
    ...
    const duplicateKey = fieldMasters.find(field => 
      field.key === key && 
      field.id !== editingId && // Exclude current field when editing
      field.isActive // Only check active fields
    )
  ...
  const handleToggleActive = async (fieldId, fieldLabel, currentStatus) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const fieldData = {
      ...
      ...(formData.type === "checkbox" && { defaultValue: formData.defaultValue })
    }
  ...
  const activeFieldsCount = fieldMasters.filter(f => f.isActive).length
  const inactiveFieldsCount = fieldMasters.filter(f => !f.isActive).length
    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto p-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-2">
          ...
          <Card className="w-full h-14 justify-center p-1">
            <CardContent className="p-2 flex gap-3 items-center">
              <div className="text-sm text-muted-foreground">Inactive Fields :</div>
          ...
          <Card className="w-full h-14 justify-center p-1">
            <CardContent className="p-2 flex gap-3 items-center">
              ...
              <div className="font-bold">
                {fieldMasters.filter(f => f.applicableFor.includes("UNIQUE") && f.isActive).length}
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  ...
                  {filterStatus === "all" && ` (${activeFieldsCount} active, ${inactiveFieldsCount} inactive)`}
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <TableBody>
                      {filteredFieldMasters.length === 0 ? (
                        ...
                        filteredFieldMasters.map((field) => (
                          <TableRow key={field.id} className={!field.isActive ? "bg-gray-50" : ""}>
                            <TableCell>
                              {field.isActive ? (
                            ...
                            <TableCell>
                              <Badge variant="outline" className="flex items-center gap-1">
                                {field.type === "select" && <ListFilter className="h-3 w-3" />}
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-2">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleToggleActive(field.id, field.label, field.isActive)}
                                  className={field.isActive ? "text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50" : "text-green-600 hover:text-green-700 hover:bg-green-50"}
          ...
          <TabsContent value="cards">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredFieldMasters.length === 0 ? (
                ...
                filteredFieldMasters.map((field) => (
                  <Card key={field.id} className={`overflow-hidden ${!field.isActive ? "border-gray-300 bg-gray-50" : ""}`}>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen} className="overflow-y-scroll">
          <DialogContent className="max-w-md overflow-y-scroll h-full">
            <DialogHeader>
              <DialogTitle>
                    ...
                    <SelectContent>
                      {FIELD_TYPES.map(type => (
                        <SelectItem key={type} value={type} className="flex items-center gap-2">
                          {type === "select" && <ListFilter className="h-4 w-4" />}
                          {type === "checkbox" && <CheckSquare className="h-4 w-4" />}
                          {type === "text" ? "Text" : 
                           type === "select" ? "Select" : 
                           type === "checkbox" ? "Checkbox" : 
                           type.charAt(0).toUpperCase() + type.slice(1)}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label className="cursor-pointer">
                      {formData.isActive ? "Field is Active" : "Field is Inactive"}
                    </Label>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Inactive fields won't appear in product forms
                  </p>
                </div>
              </div>
