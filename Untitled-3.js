...
const TransferredItems = () => {
  ...
  useEffect(() => {
    ...
    console.log("🚀 ~ TransferredItems ~ shuffled:", shuffled)
    const selected = shuffled.slice(0, qty).map(s => ({
      serialNumber: s.serialNumber,
      macAddress: s.macAddress,
      warranty: s.warranty
    }));

    console.log("🚀 ~ TransferredItems ~ selected:", selected)

    // setValue("selectedSerials", selected);
  }, [qty]);
